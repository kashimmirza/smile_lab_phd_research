...
FPR = helper_utils.get_fpr(y,pred)
TPR = helper_utils.get_tpr(y,pred)
precision = TP/(TP+FP)
...
for th in np.arange(0,1,0.01):
    tpr.append(helper_utils.get_tpr(y,pred,th))
    fpr.append(helper_utils.get_fpr(y,pred,th))
...
random_fpr = [] 
for th in np.arange(0,1,0.01):
    random_tpr.append(helper_utils.get_tpr(y,random_predictions,th))
    random_fpr.append(helper_utils.get_fpr(y,random_predictions,th))
...
FPR = helper_utils.get_fpr(y_2,pred_2)
TPR = helper_utils.get_tpr(y_2,pred_2)
precision = TP/(TP+FP)
...
fpr_2 = [] 
for th in np.arange(0,1,0.01):
    tpr_2.append(helper_utils.get_tpr(y_2,pred_2,th))
    fpr_2.append(helper_utils.get_fpr(y_2,pred_2,th))
...
for th in np.arange(0,1,0.01):
    examples_and_thresholds.append(((th,(helper_utils.get_fpr(y_2,pred_2,th),helper_utils.get_tpr(y_2,pred_2,th)))))
...
FPR = helper_utils.get_fpr(y_2,pred_2,0.08)
TPR = helper_utils.get_tpr(y_2,pred_2,0.08)
precision = TP/(TP+FP)

print(f"FPR (1 - specificity): {FPR:.4f}\nTPR (sensitivity or recall): {TPR:.4f}\nPrecision: {precision:.4f}\nSpecificity: {(1-FPR):.4f}")


# If you compare with the results in `Cell 18`, this is a nice improvement! Of course, the precision dropped down, but overall the predictions are better. There is no right answer here, if a False Positive is very expensive, we may want sacrifice the recall to increase precision, by increasing the threshold. 
